...
$\{u,u+1,\dots,v\}$, and "two distinct" blocks means the two index intervals are
...
$\{u,u+1,\dots,v\}$ with $1\le u\le v\le s$, such that
...
for the sum over the index interval $\{u,\dots,v\}$ (so $S(u,u)=a_u$). Because
...
cannot be $1$; hence $i\ge 2$. By the minimality of $i$, $T_{i-1}<T$. By Lemma 4,
...
$$\Sigma := \left\{\, (r,q) \;:\; 1\le r\le M,\; q \text{ an integer with } 0\le q < \left\lfloor \tfrac{M^{2}}{r} \right\rfloor \,\right\}.$$
...
$i(r,q) \in \{1,\dots,s-r+1\}$ whose window sum

$$\sigma(r,q) := T_{i(r,q)}$$

satisfies

$$X + q\,Kr \;\le\; \sigma(r,q) \;<\; X + (q+1)\,Kr. \tag{$\dagger$}$$
...
$$I := \{i,\,i+1,\dots,i+r-1\}, \qquad J := \{i',\,i'+1,\dots,i'+r'-1\} .$$

Both are nonempty intervals contained in $\{1,\dots,s\}$, because
...
4. **Computational spot-check (not part of the proof).** For
   $(a,K)\in\{(1,1),(2,1),(3,1),(5,1),(10,1),(1,2),(2,2),(3,2),(1,3),(2,3)\}$
